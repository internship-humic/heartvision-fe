...
export default function RegisterDoctorForm() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName || !sipNumber || !specialization || !email || !password || !confirmPassword) {
    ...
    try {
      // 1. Consume backend doctor registration
      const res = await apiFetch("/auth/register", {
        method: "POST",
        body: JSON.stringify({
          fullName,
          sipNumber,
          subspecialist: specialization,
          email,
          password,
          confirmPassword,
          role: "doctor",
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <AuthHeader title="Register Now" subtitle="Join as a verified heart specialist" />
      <form onSubmit={handleRegister} className="w-full">
        ...
        <InputField
          label="SIP Number"
          type="text"
          placeholder="Enter your SIP number"
          value={sipNumber}
          onChange={(e: any) => setSipNumber(e.target.value)}
        />
        <InputField
          label="Cardiac Subspecialist"
          type="text"
