...
export default function VerificationQueue() {
  ...
  const [filterType, setFilterType] = useState<"All" | "Verified" | "Pending">("Pending");
  ...
  useEffect(() => {
    ...
    async function loadQueue() {
      setIsLoading(true);
      try {
        const statusParam = filterType.toLowerCase();
        const res = await apiFetch(`/verifications/queue?status=${statusParam}`);
        if (res.success && res.data && Array.isArray(res.data.predictions)) {
          const mapped = res.data.predictions.map((item: any) => {
            ...
            return {
              ...
              date: item.created_at ? new Date(item.created_at).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }) : "Recently",
              findingsTitle: label,
              findingsDesc: desc,
  ...
  const totalEntries = queueList.length;
  const totalPages = Math.ceil(totalEntries / itemsPerPage) || 1;
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = queueList.slice(indexOfFirstItem, indexOfLastItem);
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-xs font-bold text-textt">
                    No results found for status &quot;{filterType}&quot;.
                  </td>
                </tr>
              ) : (
